...
# Usage: irm https://raw.githubusercontent.com/cmgzone/notebookllmmcp/master/scripts/install-mcp.ps1 | iex

$ErrorActionPreference = "Stop"

Write-Host "ðŸ“¦ Installing NotebookLLM MCP Server..." -ForegroundColor Cyan
...
# Get latest release
Write-Host "ðŸ” Finding latest release..." -ForegroundColor Yellow
try {
    ...
    Write-Host "âš ï¸  Could not fetch latest release, using fallback..." -ForegroundColor Yellow
    $version = "1.0.0"
    $downloadUrl = "https://github.com/$GITHUB_REPO/releases/latest/download/notebookllm-mcp-$version.zip"
}

Write-Host "ðŸ“¥ Downloading version $version..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "âœ… NotebookLLM MCP Server v$version installed to $MCP_DIR" -ForegroundColor Green
Write-Host ""
Write-Host "ðŸ“ Add this to your MCP config (e.g., ~/.kiro/settings/mcp.json):" -ForegroundColor Cyan
Write-Host ""
...
Write-Host ""
Write-Host "ðŸ”‘ Get your API token from Settings â†’ Agent Connections in the app" -ForegroundColor Yellow
Write-Host ""
