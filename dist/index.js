    ...
    try {
        switch (name) {
            ...
            case 'export_sources': {
                ...
                if (input.language)
                    params.append('language', input.language);
                if (input.includeVerification !== undefined)
                    params.append('includeVerification', input.includeVerification.toString());
                if (input.includeConversations !== undefined)
                    params.append('includeConversations', input.includeConversations.toString());
            ...
            case 'get_usage_stats': {
                ...
                const params = new URLSearchParams();
                if (input.period)
                    params.append('period', input.period);
            ...
            case 'list_plans': {
                ...
                if (input.includeArchived !== undefined)
                    params.append('includeArchived', input.includeArchived.toString());
                if (input.limit)
                    params.append('limit', input.limit.toString());
                if (input.offset)
                        ...
                        content: [
                            {
                                ...
                                text: JSON.stringify({
                                    ...
                                    results: results.slice(0, num || 5).map((r) => ({
                                        title: r.title,
                                        link: r.link,
                                        snippet: r.snippet,
                                        date: r.date || null,
